...
Match requests making GET for /endpoint, and then respond with status 200 and body "1", once (seen 0).
Match requests making GET for /endpoint, and then respond with status 200 and body "2/3", twice (seen 0).
Match requests making GET for /endpoint, and then respond with status 200 and body "4/5/6", thrice (seen 0).
Match requests making GET for /endpoint, and then respond with status 200 and body "7/8/9/10", 4 times (seen 0).
Match requests making GET for /endpoint, and then respond with status 200 and body "forever", always (seen 0).
`);
    });
    ...
    it("should explain whether completion rules are completed, or still waiting", async () => {
        ...
        expect(responseText).to.include(`
Match requests making GET for /endpoint, and then respond with status 200 and body "1", once (done).
Match requests making GET for /endpoint, and then respond with status 200 and body "2/3", twice (done).
Match requests making GET for /endpoint, and then respond with status 200 and body "4/5/6", thrice (done).
Match requests making GET for /endpoint, and then respond with status 200 and body "7/8/9/10", 4 times (seen 2).
Match requests making GET for /endpoint, and then respond with status 200 and body "forever", always (seen 0).
`);
    });
...
Match requests making GET matching //endpointA/\\d+/, and then respond with status 200 and body "nice request!", once (done).
Match requests making POST, for /endpointB, and with form data including {"key":"value"}, and then respond with status 500.
Match requests making POST, for /endpointC, and with a JSON body equivalent to {"key":"value"}, and then respond with status 500.
Match requests making PUT, for /endpointD, and with a query including {"a":"1"}, and then close the connection, always (seen 0).
Match requests making PUT, for /endpointE, for host abc.com, and with a query exactly matching \`?\`, and then time out (never respond).
...
`);
    ...
    it("should explain callback handlers", async () => {
        ...
        expect(text).to.include(`The configured rules are:
Match requests making POST for /endpointA, and then respond using provided callback.
Match requests making POST for /endpointB, and then respond using provided callback (handleRequest).
`);
    });
    ...
    it("should be available when inspecting endpoints", async () => {
        ...
        if (isNode) {
            ...
            expect(explanation).to.include(
                'Match requests making GET for /endpoint, and then respond with status 200 and body "first response", twice (seen 1).'
            );
            expect(explanation).to.include(
                'Match requests making GET for /endpoint, and then respond with status 200 and body "second response".'
            );
        } else {
            const explanations = endpoints.map(p => (p as any).explanation);
            expect(explanations).to.deep.equal([
                'Match requests making GET for /endpoint, and then respond with status 200 and body "first response", twice.',
                'Match requests making GET for /endpoint, and then respond with status 200 and body "second response".'
            ]);
        }
    });
